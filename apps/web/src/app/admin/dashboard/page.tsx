...
export default function DashboardPage() {
  ...
  const cards = [
    { label: 'Total Siswa', value: s?.totalSiswa ?? 0, icon: Users, sub: `${s?.siswaAktif ?? 0} aktif · ${s?.siswaAlumni ?? 0} alumni`, color: 'text-blue-600', bg: 'bg-blue-500/10' },
    { label: 'Total Pelatih', value: s?.totalPelatih ?? 0, icon: GraduationCap, color: 'text-green-600', bg: 'bg-green-500/10' },
    { label: 'Jadwal Latihan', value: s?.totalJadwal ?? 0, icon: Calendar, color: 'text-purple-600', bg: 'bg-purple-500/10' },
    { label: 'Tagihan Belum Lunas', value: s?.totalTagihanBelum ?? 0, icon: AlertCircle, color: 'text-red-600', bg: 'bg-red-500/10' },
  ...
  return (
    <div className="space-y-6 w-full max-w-6xl mx-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {cards.map((c) => (
          <Card key={c.label} className="bg-card border border-border rounded-xl shadow-2xs hover:shadow-md transition-shadow">
            <CardHeader className="flex flex-row items-center justify-between pb-2">
              <CardTitle className="text-5xs font-bold text-muted-foreground uppercase tracking-widest">{c.label}</CardTitle>
